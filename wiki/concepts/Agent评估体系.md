...
[[concepts/ReasoningEvalVerifier边界]] 把 grader 进一步拆成 outcome verifier、process verifier、trace grader、CoT monitor 和 reward model：

- outcome verifier 适合最终答案、测试和状态检查。
- process verifier 适合多步 reasoning、search pruning 和过程监督。
- trace grader 适合 agent transcript、工具调用和恢复路径。
- reward model 是训练出的评分器，也必须被独立 eval 校准。
...
[[concepts/RewardHackingEvalOverfitting边界]] 是 agent eval 的反向检查表：

- 固定公开 benchmark 只能做粗筛，不能做唯一上线门。
- LLM grader 必须用人工样本校准，并加入 adversarial prompts。
- 读 transcript 是为了发现“通过 eval 但没解决任务”的路径。
- Self-improvement loop 不能无审查地修改 eval、测试、日志或 reward。
- 产品指标要和真实用户结果、失败投诉、人工抽样一起看，避免 KPI gaming。
...
- [[concepts/RewardHackingEvalOverfitting边界]] — eval 被优化压力污染时的失效模式
